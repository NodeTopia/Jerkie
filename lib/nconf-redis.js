...
var Redis = module.exports = function (options) {
    options = options || {};
    this.type = 'redis';
    this.namespace = 'nconf' + options.namespace || 'nconf';
    ...
    this.redis = new redis({
        port: this.port,          // Redis port
        host: this.host,   // Redis host
...
Redis.prototype.get = function (key) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        self.redis.smembers(nconf.key(fullKey, 'keys'), async function (err, keys) {
            function addValue(source, next) {

                self._get(nconf.key(key, source)).then(function (data) {
                    result[source] = data;
                    next()
                })
            }
...
Redis.prototype.set = function (key, value) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        self._addKeys(key, function (err) {
            if (err) {
                return callback(err);
            }
            ...
            if (!Array.isArray(value) && value !== null && typeof value === 'object') {
                ...
                resolve(self._setObject(fullKey, value));
            }
            else {
                //
                ...
                resolve(self.redis.set(fullKey, value));
            }
        });
    })
};
