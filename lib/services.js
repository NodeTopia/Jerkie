...
class Services extends events.EventEmitter {
    ...
    async start() {
            ...
            if (fs.lstatSync(filePath).isDirectory()) {
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    require(innerFilePath).forEach(function (item) {
                        console.log(`Jerkie loading service ${item.meta.path}`);
            ...
            if (typeof method === 'string') {
                if (fs.lstatSync(method).isDirectory()) {
                    ...
                    for (let i = 0; i < files.length; i++) {
                        ...
                        console.log(`Jerkie loading method ${name}.${fileName}`);
                        this.methods[name][fileName] = require(filePath).bind(this);
                    }
                } else {
                    console.log(`Jerkie loading method ${name}`);
        ...
        if (this.jerkie.config.start) {
            console.log(`Jerkie calling start`);
    ...
    _addService(item) {
        ...
        let {meta, service, events, middleware} = item;
        ...
        async function serviceCallback(job, done) {
            ...
            let middlewares = {};
            console.log(`Service ${meta.path} has been called`)
            ...
            new Promise(async function (resolve, reject) {
                ...
                console.log(`Service ${meta.path} has thrown an error ${error.message}`)
                done(err)
            }).then(function (result) {

                console.log(`Service ${meta.path} has finished`)
        ...
        if (events) {
            for (let [event, fn] of Object.entries(events)) {
                ...
                this.jerkie.pubsub.on(event, function (data) {
                    ...
                    try {
                        new Context(self, null, self.methods, data, null, null, meta, fn, true)
                    } catch (err) {
                        //console.log(err)
                    }
